function numObjects = contarObjetosBlob3(originalImage, binaryImage, useAreaMedian)
    ...
    % Criar o objeto vision.BlobAnalysis, sem limite no número de blobs
    blobAnalyzer = vision.BlobAnalysis('BoundingBoxOutputPort', true, ...
                                       'AreaOutputPort', true, ...
                                       'CentroidOutputPort', false, ...
                                       'MinimumBlobArea', 400, ... % Ajustar o valor da área mínima
                                       'MaximumCount', 400); % Remove o limite de blobs

    % Detectar blobs, obter as áreas e caixas delimitadoras
    [areas, boxes] = step(blobAnalyzer, binaryImage);

    % Calcular a mediana das áreas, se necessário
    if useAreaMedian && ~isempty(areas)
